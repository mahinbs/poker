...
export default function AffiliateSidebar({ 
  ...
  const userEmail = user.email || staffUser.email || 'affiliate@pokerroom.com';
  const displayName = user.displayName || staffUser.displayName || 'Affiliate';
  
  // Get affiliate code from prop or localStorage
  const affiliateCodeToShow = affiliateCode || user.affiliateCode;
            ...
            >
              ...
              {clubId && (
                <div className="mt-3 pt-3 border-t border-slate-600 space-y-2">
                  ...
                  {affiliateCodeToShow && (
                    <div>
                      <p className="text-xs text-gray-400">Affiliate Code:</p>
                      <p className="font-mono text-sm text-purple-400">{affiliateCodeToShow}</p>
                    </div>
                  )}
                </div>
              )}
