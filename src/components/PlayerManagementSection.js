...
export default function PlayerManagementSection({
  ...
  // Use provided data directly (no defaults)
  const activeKycRequests = kycRequests || [];
  const activeProfileUpdates = profileUpdates || [];
  const activePendingApprovals = pendingApprovals || [];
  const activeSuspendedPlayers = suspendedPlayers || [];
  const activeAllPlayers = allPlayers || [];
