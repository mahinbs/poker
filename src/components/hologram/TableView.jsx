...
export default function TableView({ 
  ...
  const setLocation = onNavigate || ((path) => {
  ...
  const currentTable = tablesArray.find((table) => String(table.id) === String(tableId));

  // Manager mode: Get occupied seats from prop, otherwise use dummy data
  const managerOccupiedSeats = isManagerMode ? (occupiedSeats[tableId] || []) : [];
  ...
  const isUserSeated = !isManagerMode && gameStatus.isInActiveGame && gameStatus.activeGameInfo?.tableId === tableId;
  ...
  const handleJoinWaitlist = (seatNumber) => {
    if (isManagerMode && onSeatAssign && selectedPlayerForSeating) {
      ...
      onSeatAssign({
        playerId: selectedPlayerForSeating.id || selectedPlayerForSeating.playerId,
        playerName: selectedPlayerForSeating.playerName || selectedPlayerForSeating.name,
        tableId: parseInt(tableId),
        seatNumber: seatNumber
      });
      toast({
        title: "Seat Assigned!",
        description: `${selectedPlayerForSeating.playerName || selectedPlayerForSeating.name} assigned to Seat ${seatNumber}`,
      });
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 to-slate-800 text-white">
            ...
            <div className="absolute inset-0 rounded-[50%] bg-gradient-to-br from-amber-600 via-yellow-500 to-amber-600 p-2 shadow-2xl">
              ...
              <div className="absolute inset-2 rounded-[50%] bg-gradient-to-br from-emerald-600 via-emerald-700 to-emerald-800 shadow-inner">
                ...
                {Array.from({ length: currentTable.maxPlayers || 6 }, (_, index) => {
                  ...
                  const seatedPlayer = seatedPlayersArray.find((p) => p.seatNumber === seatNumber);
                  ...
                  const isPreferredSeat = isManagerMode && selectedPlayerForSeating 
                    ? (selectedPlayerForSeating.preferredSeat === seatNumber && 
                       String(selectedPlayerForSeating.preferredTable) === String(tableId))
                    : false;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div 
                        ...
                        title={isManagerMode && isPreferredSeat ? `Preferred seat for ${selectedPlayerForSeating.playerName || selectedPlayerForSeating.name}` : `Seat ${seatNumber}`}
                      >
                      ...
                      <div className={`absolute -bottom-8 left-1/2 transform -translate-x-1/2 text-center transition-colors ${
                        ...
                        <div className="text-xs font-medium">
                          {isOccupied && seatedPlayer?.player ? seatedPlayer.player.firstName : `Seat ${seatNumber}`}
                        </div>
                        {isPreferredSeat && (
        ...
        {!isUserSeated && (
          <div className="mt-8 text-center">
            {selectedSeat ? (
              ...
              <div className={`bg-slate-800 border rounded-lg p-4 max-w-md mx-auto ${
                isManagerMode ? 'border-yellow-500/50' : 'border-emerald-500/50'
              }`}>
                <h3 className={`font-semibold mb-2 ${
                  isManagerMode ? 'text-yellow-400' : 'text-emerald-400'
                }`}>
                  {isManagerMode ? `Assign Seat ${selectedSeat}` : `Seat ${selectedSeat} Selected`}
                </h3>
                {isManagerMode && selectedPlayerForSeating ? (
                  <>
                    <p className="text-slate-300 text-sm mb-2">
                      Assign <span className="font-semibold text-white">{selectedPlayerForSeating.playerName || selectedPlayerForSeating.name}</span> to Seat {selectedSeat}
                    </p>
                ...
                <div className="flex gap-3 justify-center">
                  ...
                  >
                    ...
                    {isManagerMode ? `Assign Seat ${selectedSeat}` : `Reserve Seat ${selectedSeat}`}
                  </Button>
                  <Button
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-4 max-w-md mx-auto">
              <p className="text-slate-300 text-sm">
                {isManagerMode && selectedPlayerForSeating
                  ? `Click on any available seat to assign ${selectedPlayerForSeating.playerName || selectedPlayerForSeating.name}`
                  : "Click on any seat to reserve your preferred position"}
              </p>
              <p className="text-slate-400 text-xs mt-2">
                {isManagerMode ? "Seats highlighted in yellow are preferred seats for this player" : "Staff will manage final table assignments"}
              </p>
            </div>
          )}
          </div>
      ...
      <Dialog open={showJoinDialog} onOpenChange={setShowJoinDialog}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white">
          <DialogHeader>
            <DialogTitle className={`text-xl ${isManagerMode ? 'text-yellow-400' : 'text-emerald-400'}`}>
              {isManagerMode ? 'Assign Seat' : 'Join Table Waitlist'}
            </DialogTitle>
            <DialogDescription className="text-slate-300">
              {isManagerMode 
                ? `Confirm seat assignment for ${selectedPlayerForSeating?.playerName || selectedPlayerForSeating?.name || 'player'}`
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
              <h4 className={`font-semibold mb-2 ${isManagerMode ? 'text-yellow-400' : 'text-emerald-400'}`}>
                {isManagerMode ? 'Assignment Information' : 'Table Information'}
              </h4>
              <div className="text-sm space-y-1 text-slate-300">
                {isManagerMode && selectedPlayerForSeating && (
                  <>
                    <div>• Player: <span className="text-white font-semibold">{selectedPlayerForSeating.playerName || selectedPlayerForSeating.name}</span></div>
                    <div>• Player ID: {selectedPlayerForSeating.playerId || selectedPlayerForSeating.id}</div>
                    {selectedPlayerForSeating.preferredSeat && (
                      <div>• Preferred Seat: {selectedPlayerForSeating.preferredSeat === selectedSeat 
                        ? <span className="text-yellow-400 font-semibold">✓ Seat {selectedSeat} (Matches preference)</span>
                        : <span className="text-gray-400">Seat {selectedPlayerForSeating.preferredSeat}</span>
                      }</div>
                    )}
                  </>
                )}
                ...
                <div>• Selected Seat: <span className="text-white font-semibold">{selectedSeat}</span></div>
              </div>
            </div>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                ...
                className={isManagerMode 
                  ? "bg-gradient-to-r from-yellow-600 to-yellow-700 hover:from-yellow-700 hover:to-yellow-800 text-white"
                  : "bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white"
                }
              >
                <UserPlus className="w-4 h-4 mr-2" />
                {isManagerMode ? `Assign Seat ${selectedSeat}` : `Confirm Seat ${selectedSeat}`}
              </Button>
            </div>
          </div>
        </DialogContent>
