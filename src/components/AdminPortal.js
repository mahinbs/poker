...
export default function AdminPortal() {
  ...
  const validateVideoUrl = (url) => {
    ...
    if (!videoUrlPattern.test(url)) return "Please enter a valid video URL (YouTube, Vimeo, DailyMotion, Facebook, Instagram)";
    return null;
  };
  ...
  const handleSendNotification = () => {
    ...
    if (notificationForm.imageFile && notificationForm.imageUrl) errors.image = "Please use either image upload OR image URL, not both";
    ...
    if (notificationForm.imageFile) {
      ...
      payload.media.imageFile = notificationForm.imageFile;
  ...
  const handleStaffBonusAction = (requestId, action, notes = "") => {
    setStaffBonusRequests(prev => prev.map(req => {
      if (req.id === requestId) {
        if (action === "approve") {
          return { ...req, status: "approved", approvedDate: new Date().toISOString(), approvedBy: "Super Admin", notes: notes };
        } else {
          return { ...req, status: "rejected", rejectedDate: new Date().toISOString(), rejectedBy: "Super Admin", notes: notes };
        }
      }
      return req;
    }));
  ...
  const handleExportCSV = () => {
    const csvContent = [
      ...
      ...filteredRegisteredPlayers.map(p => [
        p.id, p.name, p.email, p.phone, p.accountStatus, p.registrationDate, p.verifiedDate, p.documentType
      ])
  ...
  // Get all players for report search
  ...
  // Player search for Credit Management
  const [creditManagementSearch, setCreditManagementSearch] = useState("");
  const [selectedCreditPlayer, setSelectedCreditPlayer] = useState(null);
  const filteredCreditPlayers = creditManagementSearch.length >= 3
    ? allPlayersForReport.filter(player => {
        const searchLower = creditManagementSearch.toLowerCase();
        return (
          player.name.toLowerCase().includes(searchLower) ||
          player.id.toLowerCase().includes(searchLower) ||
          player.email.toLowerCase().includes(searchLower)
                  ...
                  <div className="space-y-3">
                    <div className="relative">
                      ...
                      <input 
                        ...
                        value={creditManagementSearch}
                        onChange={(e) => {
                          setCreditManagementSearch(e.target.value);
                          setSelectedCreditPlayer(null);
                        }}
                      />
                      {creditManagementSearch.length >= 3 && filteredCreditPlayers.length > 0 && !selectedCreditPlayer && (
                        <div className="absolute z-10 w-full mt-1 bg-gray-800 border border-white/20 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                          {filteredCreditPlayers.map(player => (
                            <div
                              key={player.id}
                              onClick={() => {
                                setSelectedCreditPlayer(player);
                                setCreditManagementSearch(`${player.name} (${player.id})`);
                              }}
                      ...
                      {selectedCreditPlayer && (
                        <div className="mt-2 p-2 bg-green-500/20 border border-green-400/30 rounded text-sm">
                          <span className="text-green-300">Selected: {selectedCreditPlayer.name} ({selectedCreditPlayer.id})</span>
                          <button 
                            onClick={() => {
                              setSelectedCreditPlayer(null);
                              setCreditManagementSearch("");
                            }}
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => {
                          if (!selectedCreditPlayer) {
                            alert("Please select a player first");
                            return;
                          }
                          alert(`Adding credit for ${selectedCreditPlayer.name} (${selectedCreditPlayer.id})`);
                        }}
                      ...
                      <button 
                        onClick={() => {
                          ...
                          alert(`Deducting credit for ${selectedCreditPlayer.name} (${selectedCreditPlayer.id})`);
                        }}
                    ...
                    <div className="p-6 space-y-6">
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div><label className="text-gray-400 text-sm">Account Status</label><div>
                              <span className={`px-3 py-1 rounded-full text-xs border font-medium ${
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div><label className="text-gray-400 text-sm">KYC Status</label><div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white/10 p-4 rounded-lg">
                    ...
                    <div className="space-y-3">
                      ...
                      <button className="w-full bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-lg font-semibold mt-4">
                        Create Table
                      </button>
                    </div>
                  </div>
          ...
          {activeItem === "Player Registration" && (
            <div className="space-y-6">
              ...
              <section className="p-6 bg-gradient-to-r from-indigo-600/30 via-purple-500/20 to-pink-700/30 rounded-xl shadow-md border border-indigo-800/40">
                <h2 className="text-xl font-bold text-white mb-6">Reports & Analytics - Comprehensive Report System</h2>
                <div className="text-white/80 mb-6 text-sm">
                  All data is available at the club's discretion. Generate, save, edit, and export reports in CSV or PDF format.
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="bg-white/10 p-4 rounded-lg">
                ...
                {/* Report Types Grid and Saved Reports - Additional sections would go here */}
                  ...
                  <div className="mt-6 bg-white/10 p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      {savedReports.map(report => (
                        <div key={report.id} className="bg-white/5 p-3 rounded border border-white/10 flex justify-between items-center">
                          <div>
                            <div className="text-white font-semibold">{report.name}</div>
                            <div className="text-xs text-gray-400">{report.dateRange}</div>
                          </div>
                          <div className="flex gap-2">
                            <button onClick={() => handleExportReportCSV(report.type, reportData)} className="bg-green-600 hover:bg-green-500 text-white px-2 py-1 rounded text-xs">CSV</button>
                            <button onClick={() => handleExportReportPDF(report.type)} className="bg-blue-600 hover:bg-blue-500 text-white px-2 py-1 rounded text-xs">PDF</button>
                            <button onClick={() => {
                              const newName = prompt("Edit name:", report.name);
                              if (newName) {
                                setSavedReports(prev => prev.map(r => r.id === report.id ? {...r, name: newName} : r));
                              }
                            }} className="bg-yellow-600 hover:bg-yellow-500 text-white px-2 py-1 rounded text-xs">Edit</button>
                            <button onClick={() => {
                              if (window.confirm(`Delete "${report.name}"?`)) {
                                setSavedReports(prev => prev.filter(r => r.id !== report.id));
                              }
                            }} className="bg-red-600 hover:bg-red-500 text-white px-2 py-1 rounded text-xs">Delete</button>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white/10 p-4 rounded-lg">
                    ...
                    {selectedTable && selectedTable.status === "Active" ? (
                      <div className="space-y-3">
                        ...
                        <button className="w-full bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-semibold mt-2">
                          Save Parameters
                        </button>
                      </div>
                    ) : (
                  ...
                  <div className="bg-white/10 p-4 rounded-lg">
                    ...
                    <div className="space-y-4">
                      ...
                      <button 
                        ...
                        className="w-full bg-violet-600 hover:bg-violet-500 text-white px-4 py-2 rounded-lg font-semibold mt-4"
                      >
                        Send Notification
                      </button>
                    </div>
