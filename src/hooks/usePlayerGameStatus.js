...
export function usePlayerGameStatus() {
  ...
  let activeGameInfo = undefined;
  let canJoinWaitlists = true;
  let restrictionMessage = undefined;
  let seatedSessionFallback = undefined;
  ...
  const activeSeat = seatedSessions.find((seat) => seat.status === 'seated') ||
                    seatRequestsArray.find((req) => req.status === 'seated');

  if (activeSeat) {
    // Player is seated somewhere, check if the table is active
    const tableInfo = Array.isArray(tables) ? tables.find((t) => t.id === activeSeat.tableId) : null;
    ...
    for (const entry of waitlistArray) {
      const tableInfo = Array.isArray(tables) ? tables.find((t) => t.id === entry.tableId) : null;
